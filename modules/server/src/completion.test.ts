...
describe('GosuCompletionProvider', () => {
  ...
  const CLASS_CONTEXT_DOCUMENT = 'package test\n\nc'
  const CLASS_BODY_DOCUMENT = `package test

public class TestClass {
  f
}`
  const VISIBILITY_MODIFIER_DOCUMENT = `package test

public class TestClass {
  pr
}`
  const FUNCTION_BODY_DOCUMENT = `package test
...
public class TestClass {
  ...
  const LITERAL_CONTEXT_DOCUMENT = `package test
...
public class TestClass {
  ...
  const NO_PREFIX_DOCUMENT = 'package test\n\n'
  ...
  const logContext = (context: string, position: Position, prefix?: string) => {
    debug(`${context}: Testing at position ${position.line}:${position.character}${prefix ? ` with prefix "${prefix}"` : ''}`)
  }
