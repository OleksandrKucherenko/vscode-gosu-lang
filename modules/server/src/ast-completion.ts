...
import { type CompletionItem, CompletionItemKind, type Position, type TextDocument } from "vscode-languageserver/node"
...
export class GosuASTCompletionProvider {
   ...
   */
  private getMemberAccessCompletions(_symbolTable: GosuSymbolTable, prefix: string, _line: number): CompletionItem[] {
   ...
   */
  private async getImportCompletions(symbolTable: GosuSymbolTable, prefix: string): Promise<CompletionItem[]> {
   ...
   */
  private getGeneralCompletions(symbolTable: GosuSymbolTable, prefix: string, line: number): CompletionItem[] {
