...
describe('GosuASTCompletionProvider', () => {
  ...
  beforeEach(() => {
    provider = new GosuASTCompletionProvider()
  })
  ...
  describe('Error handling', () => {
    test('should handle malformed document gracefully', async () => {
      const content = 'invalid gosu code {'
    ...
    test('should handle parser errors in getOrUpdateSymbolTable', async () => {
      const content = 'class Missing { // unclosed class'
  ...
  describe('Import completions coverage', () => {
    test('should provide filtered import completions with specific prefix', async () => {
      ...
      // Should include java.util imports (covering lines 278-286)
      const utilImports = completions.filter(c => c.label.includes('java.util'))
      expect(utilImports.length).toBeGreaterThan(0)
      
      // Verify completion properties (covering line 280-286)
      utilImports.forEach(completion => {
        expect(completion.kind).toBe(CompletionItemKind.Module)
        expect(completion.detail).toBe('import')
        expect(completion.sortText).toContain('5000_')
      })
    })
      ...
      const content = `
        ...
        class MyClass {
          function method() : void {
            var local : MyClass  // This creates context where MyClass could appear multiple times
          }
        }
      `
  ...
  describe('Completion context edge cases', () => {
    test('should handle member access without duplicates check', async () => {
      ...
      const content = `
        ...
        class SameNameClass {
          var SameNameField : String
          
          function methodWithSameName() : void {
            var SameNameField : String  // Local var with same name as field
            SameNameField.  // This should trigger member access
          }
        }
      `
