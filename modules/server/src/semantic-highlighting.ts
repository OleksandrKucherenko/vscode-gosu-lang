...
import { GosuSymbolTable } from '@gosu-lsp/shared';
import Debug from 'debug';

const debugLog = Debug('gosu:lsp:semantic');
...
export class GosuSemanticHighlightingProvider {
  ...
  async getSemanticTokens(document: TextDocument): Promise<SemanticTokens> {
    debugLog(`Getting semantic tokens for ${document.uri}`);

    // Check cache first
    const cached = this.cache.get(document.uri);
    if (cached && cached.version === document.version) {
      debugLog(`Using cached tokens for ${document.uri}`);
    ...
    try {
      ...
      debugLog(`Computed ${tokens.data.length / 5} tokens for ${document.uri}`);
      return tokens;
      
    } catch (error) {
      debugLog(`Error computing semantic tokens for ${document.uri}:`, error);
  ...
  async getSemanticTokensRange(document: TextDocument, range: Range): Promise<SemanticTokens> {
    debugLog(`Getting semantic tokens for range in ${document.uri}`);

    try {
      const allTokens = await this.getSemanticTokens(document);
      const filteredTokens = this.filterTokensInRange(allTokens, range);
      
      debugLog(`Filtered to ${filteredTokens.data.length / 5} tokens for range`);
      return filteredTokens;
      
    } catch (error) {
      debugLog(`Error computing range tokens for ${document.uri}:`, error);
      return { data: [] };
    }
  }
   ...
   */
  clearAllCaches(): void {
    debugLog('Clearing all semantic highlighting caches');
    this.cache.clear();
  }
  ...
  private async computeSemanticTokens(document: TextDocument): Promise<SemanticTokens> {
    ...
    try {
      ...
      debugLog(`Parse error, falling back to basic tokenization:`, error);
   ...
   */
  private processLine(line: string, lineIndex: number, builder: SemanticTokensBuilder, symbolTable: GosuSymbolTable): void {
    // Keywords
  ...
  private highlightKeywords(line: string, lineIndex: number, builder: SemanticTokensBuilder): void {
    const keywords = [
      'package', 'uses', 'class', 'interface', 'enum', 'enhancement', 'structure',
      'function', 'property', 'var', 'final', 'static', 'private', 'protected', 'public',
      'abstract', 'override', 'construct', 'delegate', 'implements', 'extends',
      'if', 'else', 'while', 'for', 'do', 'switch', 'case', 'default', 'break', 'continue',
      'return', 'throw', 'try', 'catch', 'finally', 'using',
      'new', 'this', 'super', 'typeof', 'typeis', 'as', 'instanceof',
   ...
   */
  private highlightSymbols(line: string, lineIndex: number, builder: SemanticTokensBuilder, symbolTable: GosuSymbolTable): void {
